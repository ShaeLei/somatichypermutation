...
def SeqDiff(seq1, seq2):
    ''' string difference between two sequences '''
    diff = ''
    ...
    def __init__(self, seq, id):
        ...
        for i in range(len(seq)):
            if seq[i] == 'A':
                ...
                self.G.add(i)
        self.size = len(self.A)+len(self.T)+len(self.C)+len(self.G)
    def __str__(self):
...
def FindPaths2(g, nodes):
    graph = nx.DiGraph(g)
    paths = []
    def _PathScore(graph, nodes, path):
        weight = nodes[path[0]].size
        #weight = 0
        for i in range(len(path)-1):
            weight += graph[path[i]][path[i+1]]['weight']
        #return len(path)
        #return weight)
        return len(path), weight, nodes[path[0]].size
    ...
    while graph.size()>0:
        ...
        for start in [x for x in graph.nodes() if graph.in_degree(x) == 0]:
            for end in [x for x in graph.nodes() if graph.out_degree(x) == 0]:
                ...
                candpaths.append(maxpath)
        #print 'cand',candpaths
        candscores = []
        for path in candpaths:
            candscores.append(_PathScore(graph, nodes, path))
        #print sorted(zip(candscores,candpaths))
        thepath = sorted(zip(candscores,candpaths))[-1][1]
        #print 'the', thepath
        #print '-'*30
        paths.append(thepath)
        graph.remove_node(paths[-1][-1])
